...
import { 
  ...
  CheckBadgeIcon, UserCircleIcon, ArrowRightStartOnRectangleIcon 
...
export default function MenuNavigation() {
  ...
  // ESTILOS COM LARGURA FORÇADA (w-full)
  const panelClass = "w-full mt-24 rounded-3xl overflow-hidden backdrop-blur-xl transition-all duration-500 bg-black/60 border border-white/10 dark:bg-cyan-900/20 dark:border-cyan-400/30 shadow-2xl";
  ...
  return (
    <div className={panelClass}>
      <div className="px-6 pt-7 pb-4">
        <p className="text-sm text-white/50 tracking-widest uppercase">{t("footer.powered")}</p>
      </div>

      <nav className="px-4 sm:px-6 pb-6 min-h-[350px]">
        <AnimatePresence mode="wait">
          {!isOptionsOpen ? (
            <motion.ul key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-3 w-full">
              ...
              <li className="w-full" onMouseEnter={() => setIndex(0)} onClick={() => !isLoggedIn && setPickerOpen(true)}>
                <div className={`${cardBase} ${index === 0 ? cardSelected : ""}`}>
                  ...
                  <span className="truncate pr-2">{isLoggedIn ? `${session?.user?.name || 'User'} Lv.1` : t("menu.new")}</span>
                  
                  {!isLoggedIn && pickerOpen ? (
                    <div className="flex items-center gap-2 bg-black/80 p-1 rounded-lg border border-white/10 shrink-0">
                      <ChevronLeftIcon className="w-4 h-4" onClick={(e) => { e.stopPropagation(); setRoleIndex(r => (r - 1 + ROLES.length) % ROLES.length); }} />
                      ...
                      <ChevronRightIcon className="w-4 h-4" onClick={(e) => { e.stopPropagation(); setRoleIndex(r => (r + 1) % ROLES.length); }} />
                    </div>
                ...
                return (
                  <li key={item.labelKey} className="w-full" onMouseEnter={() => setIndex(i + 1)} onClick={() => {
                    ...
                    <div className={`${cardBase} ${isSel ? cardSelected : ""}`}>
                      ...
                      <span className="truncate pr-2">{t(item.labelKey)}</span>
                      <ChevronRightIcon className="h-5 w-5 opacity-40 shrink-0" />
                    </div>
                  </li>
                );
            ...
            <motion.div key="options" initial={{ x: 20 }} animate={{ x: 0 }} className="flex flex-col gap-4 w-full">
